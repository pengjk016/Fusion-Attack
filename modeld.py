...
def main(demo=False):
  ...
  while True:
    ...
    #
    # # YUV I420 格式：总像素数 = width * height * 3/2（Y 通道占 1/2，U/V 各占 1/4）
    # y_size = img_width * img_height
    # uv_size = y_size // 2
    #
    # # 解析 Y、U、V 通道（numpy 数组）
    # y = np.frombuffer(yuv_data[:y_size], dtype=np.uint8).reshape((img_height, img_width))
    # u = np.frombuffer(yuv_data[y_size:y_size + uv_size // 2], dtype=np.uint8).reshape((img_height // 2, img_width // 2))
    # v = np.frombuffer(yuv_data[y_size + uv_size // 2:], dtype=np.uint8).reshape((img_height // 2, img_width // 2))
    #
    # # 步骤3：YUV I420 → BGR（OpenCV 内置函数）
    # # 先将 U/V 通道上采样到与 Y 通道相同尺寸
