#能确定patch，但是还没有优化patch,用作和optimpatch对比
# -*- coding: utf-8 -*-
"""
...
# ====== 固定路径（按你的项目）======
RESULTS_PATH       = 'results/img_test/image_eval_withoutpatch_wide_not_zeros.csv'
...
def optimize_patch_finite_diff(sess, img1_yuv, img2_yuv, h_bounds, w_bounds, initial_patch,
    ...
    """
    Finite-difference optimization of a Y-plane patch.
    - sess: onnxruntime InferenceSession
    - img1_yuv, img2_yuv: parsed YUV arrays from read_image() for frame t-1 and t (6, H/2, W/2)
    - h_bounds, w_bounds: patch region in Y-plane coords
    - initial_patch: numpy float32 shape (ph, pw)
    - layout: layout dict for parse_outputs_to_drel
    - out0_name: sess.get_outputs()[0].name
    - steps: number of FD update iterations
    - eps: FD perturbation (same units as Y values), typical 0.5~2.0
    ...
    for it in range(steps):
        ...
        for idx in chosen:
            ...
            p_plus2 = p_plus.reshape(ph, pw)
            p_minus2 = p_minus.reshape(ph, pw)
        ...
        # assemble full gradient array (zeros for non-chosen indices)
        grad2 = grad.reshape(ph, pw)

        # update patch: if maximize True do ascent
        if maximize:
        ...
        # here clamp to reasonable Y range delta; if your patches represented signed offsets [-thres,+thres]
        # adjust clamp accordingly. We'll assume patch should stay in [-thres, thres] if provided as initial.
        # If no thres info, clamp to [0,255] delta effect? We'll assume initial_patch built via build_yuv_patch within [-thres,thres]
        # So keep clamping to same min/max as initial values:
        minv = initial_patch.min()
        maxv = initial_patch.max()
        patch = np.clip(patch, minv, maxv)

        # Evaluate current loss with new patch
        img2_cur = apply_y_patch_to_yuv(img2_yuv, patch, h_bounds, w_bounds)
        ...
        if verbose:
            dt = time.time() - t0
            print(f"[FD it {it+1}/{steps}] loss={loss_cur:.4f} (lr={lr}, eps={eps}, samples={len(chosen)}) took {dt:.2f}s")
...
def main():
    ...
    with open(RESULTS_PATH, "w", newline="") as f:
        ...
        w.writerow([
            ...
            "dRel0", "dRel2", "dRel4", "dRel6", "dRel8", "dRel10",
        ])
...
#             feed_opt = make_feed(sess, img1_yuv, img2_opt)
