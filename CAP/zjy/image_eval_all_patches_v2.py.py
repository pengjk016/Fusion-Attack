...
def main():
    ...
    print("=" * 50)
    t0 = time.time()
    clean_drels, _ = evaluate_sequence(sess, yolo, image_paths, patch=None, verbose=False)
    ...
    for rank, res in enumerate(results):
        writer.writerow([rank + 1, res['name'],
                         f"{res['mean']:.4f}", f"{res['median']:.4f}",
                         f"{res['std']:.4f}", f"{res['min']:.4f}", f"{res['score']:.4f}"])
