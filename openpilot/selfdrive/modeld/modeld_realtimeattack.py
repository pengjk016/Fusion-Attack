...
def main(demo=False):
  ...
  while True:
    ...
    if ENABLE_PATCH and target_box is not None:
      ...
      if box_h > 0 and box_w > 0:
        ...
        if x1 >= 0 and y1 >= 0 and x2 <= img_width and y2 <= img_height:
          roi = bgr_img[y1:y2, x1:x2].astype(np.float32)
          blended = np.clip(roi + resized_patch, 0, 255).astype(np.uint8)
          bgr_img[y1:y2, x1:x2] = blended
