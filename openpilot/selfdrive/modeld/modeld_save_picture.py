...
#
#
SAVE_DIR = "/home/pjk/PycharmProjects/openpilot0.9.6/CAP/data/imgs/test_optim_patch"
...
def main(demo=False):
  ...
  while True:
    ...
    # -------------------------- 2. YOLO 检测车辆框（原有逻辑优化） --------------------------
    results = yolo_model(bgr_img, conf=0.1, verbose=False)
    ...
    if target_box is not None:
      ...
      # -------------------------- 4. BGR → NV12 (正确提取平面，避免色差) --------------------------
      # BGR → I420 (OpenCV 返回 (h*1.5, w) 形状)
    ...
    if run_count>1 and run_count<800:
      fname = f"{SAVE_DIR}/{run_count}.png"
      save_frame(buf_main, fname)
